...
public class RestControllerExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleHttpMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex,
		...
		return buildResponseEntity(
				new ErrorMessage(HttpStatus.UNSUPPORTED_MEDIA_TYPE, builder.substring(0, builder.length() - 2), ex));
	}
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
			HttpHeaders headers, HttpStatus status, WebRequest request) {
		ErrorMessage apiError = new ErrorMessage(BAD_REQUEST);
	...
	protected ResponseEntity<Object> handleHttpMessageNotWritable(HttpMessageNotWritableException ex,
		...
		return buildResponseEntity(new ErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR, error, ex));
	}
	...
	protected ResponseEntity<Object> handleDataIntegrityViolation(DataIntegrityViolationException ex,
		...
		if (ex.getCause() instanceof ConstraintViolationException) {
			return buildResponseEntity(new ErrorMessage(HttpStatus.CONFLICT, "Database error", ex.getCause()));
		}
		return buildResponseEntity(new ErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR, ex));
	}
